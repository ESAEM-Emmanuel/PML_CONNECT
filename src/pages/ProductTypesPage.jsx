// src/pages/ProductTypesPage.jsx
...
export default function ProductTypesPage() {
  ...
  const fields = useMemo(
    () => [
      { name: 'label', label: t('crud.name'), type: 'text', required: true },
      { name: 'photo', label: t('crud.media'), type: 'file' },
      { name: 'description', label: t('crud.description'), type: 'text' },
    ],
    [t]
  );
  ...
  const ViewFields = useMemo(
    () => [
      { name: 'label', label: t('crud.name') },
      { name: 'description', label: t('crud.description') },
      ...
      { name: 'photo', label: t('crud.image'), accessor: (item) => item.photo ? <img src={item.photo} alt="Image" className="w-16 h-16 object-contain" /> : 'N/A' },
      ...
      {
        name: 'products',
        label: `${t('crud.product')}(s)`,
        accessor: (item) => {
          if (!item.products || item.products.length === 0) return t('crud.no_elements');
          const names = item.products.map((product) => product.name).join(', ');
          return `${item.products.length} ${t('crud.Product')}(s) : ${names}`;
        },
      },
    ],
    [t]
  ...
  const {
    ...
    softDelete,
    restore,
  } = useCrudPaginated(productTypesService, 'product_types', queryParams);
  ...
  const { data: itemDetails, isLoading: isLoadingDetails } = useQuery({
    ...
    queryFn: () => productTypesService.getById(selectedItemId),
  ...
  const openModal = useCallback((mode, item = null) => {
    ...
    if (mode === 'edit' && item) {
      initialFormState = { label: item.label, description: item.description };
      itemId = item.id;
...
//                         // Upload des fichiers
//                         const formData = new FormData();
//                         data.photo.forEach(file => formData.append('files', file));
//                         const uploadResponse = await uploadFilesService.createFiles(formData);
...
//             const payload = { label: data.label, description: data.description || '',photo: mediaPayload[0] || '', };
...
//         }
//     },
//     [modal.mode, modal.id, create, update, uploadFilesService.createFiles, closeModal]
//   );
    ...
    const handleSave = useCallback(
        ...
        try {
            ...
            if (data.photo instanceof File) {
                ...
                mediaUrls = data.photo; // URLs existantes
                }
            }
            }
    
            // --- Construction dynamique du payload ---
            let payload = { label: data.label };
    
            if (data.description) {
            payload.description = data.description;
            }
    
            // ⚠️ N'ajouter photo que si une nouvelle existe
            if (mediaUrls.length > 0) {
            payload.photo = mediaUrls[0];
            }
    
            if (modal.mode === "create") {
            // en création → si photo est optionnelle tu peux aussi forcer
            payload.photo = mediaUrls[0] || "";
            create(payload);
            } else {
  ...
  const filterFields = useMemo(
    () => [
      ...
      { name: 'label', label: t('crud.name'), type: 'text' },
      { name: 'description', label: t('crud.description'), type: 'text' },
      ...
      { name: 'updatorId', label: t('crud.updator'), type: 'autocomplete' },
      {
        ...
        options: [
          ...
          { value: 'label', label: t('crud.name') },
          { value: 'description', label: t('crud.description') },
  ...
  const excelData = useMemo(
    () =>
      items.map((item) => ({
        ...
        products: item.products?.map((product) => product.name).join(', ') || t('crud.no_elements'),
      })),
    [items, t]
  );
  ...
  const excelHeaders = useMemo(
    () => [
      ...
      { label: t('crud.name'), key: 'label' },
      { label: t('crud.description'), key: 'description' },
      ...
      { label: `${t('crud.product')}(s)`, key: 'products' },
    ],
    [t]
  );
      ...
      <div className="p-4 max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold">{t('modul_title.product_type')}</h1>
          ...
          <table className="table w-full">
            <thead>
              <tr>
                ...
                <th>{t('crud.name')}</th>
                <th>{t('crud.description')}</th>
                <th>{t('crud.product')}(s)</th>
