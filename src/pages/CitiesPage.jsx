...
//           return `(${item.citizens?.length ?? 0})  ${names}`;
//         //   return `${item.citizens.length} ${t('crud.citizen')}(s) : ${names}`;
//         },
//       },
//       {
//         name: 'companies',
//         label: `${t('crud.company')}(s)`,
//         accessor: (item) => {
//           if (!item.companies || item.companies.length === 0) {
//             return t('crud.no_elements');
//           }
//           const names = item.companies.map((company) => company.name).join(', ');
//           return `(${item.companies?.length ?? 0})   ${names}`;
//         },
//       },
//     ],
//     [t]
...
//         companies: item.companies?.map((citizen) => citizen.name).join(', ') || t('crud.no_elements'),
//       })),
//     [items, t]
//   );
...
//       { label: `${t('crud.company')}(s)`, key: 'companies' },
//     ],
//     [t]
//   );
...
export default function CitiesPage() {
    ...
    const userOptions = useMemo(() => 
        usersData?.data?.result?.data?.map(user => ({ id: user.id, name: user.username })) || [], 
        [usersData]
    );

    // CRÉATION DE L'OBJET DE MAPPING POUR LES OPTIONS D'AUTOCOMPLÉTION
    const autocompleteOptions = useMemo(() => ({
        countryId: countryOptions,
        creatorId: userOptions,
        updatorId: userOptions,
    ...
    // CHAMPS POUR LA MODALE DE CRÉATION/ÉDITION
    const fields = useMemo(
        () => [
            { name: 'name', label: t('crud.name'), type: 'text', required: true },
            { 
    ...
    // HOOK POUR GÉRER L'ÉTAT DES FILTRES DE L'URL
    const {
        searchParams,
        searchTerm,
        handleSearchChange,
    ...
    // RÉCUPÉRATION DES DÉTAILS D'UNE VILLE POUR LA VUE "VIEW"
    const { data: itemDetails, isLoading: isLoadingDetails } = useQuery({
        queryKey: ['town', selectedItemId],
        queryFn: () => townsService.getById(selectedItemId),
        enabled: !!selectedItemId,
    });
    ...
    const excelData = useMemo(
        () =>
            items.map((item) => ({
                ...
                updatedAt: new Date(item.updatedAt).toLocaleString(),
                citizens: item.citizens?.map((citizen) => citizen.username).join(', ') || t('crud.no_elements'),
                companies: item.companies?.map((company) => company.name).join(', ') || t('crud.no_elements'),
            })),
        [items, t]
    );
    ...
    const excelFilename = useMemo(() => `cities-${new Date().toISOString().slice(0, 10)}.csv`, []);
    ...
    if (isLoading || isUsersLoading) return <p className="text-center mt-10">{t('loading')}</p>;
